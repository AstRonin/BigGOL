...
-define(BRD, 10).
-define(ARC_R, 10).
-define(RECTANGLE_SIDE, 10).

%% API
-export([new/0, set_alive/2]).
...
-record(gol_gui_state, {frame, win, parent, board = [], pen, fonts = [], alive_cells}).
...
%%    wxWindow:setMaxSize(Win, {400, 400}),
    wxWindow:setMinSize(Win, {300, 300}),
...
redraw(#gol_gui_state{win = Win, alive_cells = AliveCells} = State) ->

%%    gol_utils:log_info("redraw ~p", [AliveCells]),
...
draw_cell_field(DC, #gol_gui_state{pen = Pen} = S) ->
    #{rows := Rows, cols := Cols} = gol_utils:config(field),

    wxDC:setPen(DC, Pen),
    wxDC:drawRectangle(DC, {?BRD, ?BRD, ?RECTANGLE_SIDE * Cols, ?RECTANGLE_SIDE * Rows}),

    draw_cells(Rows - 1, Cols - 1, 0, DC, S).

draw_cells(-1, _Col, _Layer, _DC, _S) -> ok;
draw_cells(Row, Col, _Layer, DC, S) ->
    draw_row(Row, Col, _Layer, DC, S),
    draw_cells(Row - 1, Col, _Layer, DC, S).

draw_row(_Row, -1, _Layer, _DC, _S) -> ok;
draw_row(Row, Col, Layer, DC, S) ->
...
%%            wxPen:setColour(Pen, {0, 0, 0}),
            ...
            wxDC:drawRectangle(DC, {?BRD + ?RECTANGLE_SIDE * Col, ?BRD + ?RECTANGLE_SIDE * Row, ?RECTANGLE_SIDE, ?RECTANGLE_SIDE}),
...
-spec(handle_cast(Request :: term(), State :: #gol_gui_state{}) ->
    ...
    {noreply, NewState :: #gol_gui_state{}, timeout() | hibernate} |
    {stop, Reason :: term(), NewState :: #gol_gui_state{}}).
handle_cast({set_alive, AliveCells}, State) ->

%%    gol_utils:log_info("set_alive ~p: ", [AliveCells]),

    State1 = State#gol_gui_state{alive_cells = AliveCells},
    redraw(State1),
...
handle_event(#wx{event = #wxSize{}}, #gol_gui_state{alive_cells = undefined} = State) ->
    {noreply, State};
